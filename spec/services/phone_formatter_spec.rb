...
describe PhoneFormatter do
  describe '.format' do
    context 'Brazilian mobile numbers - Story 2.2 Scenario 1' do
      it 'formats number without country code to E.164' do
        expect(PhoneFormatter.format('11987654321')).to eq('+5511987654321')
      end

      it 'formats 10-digit Brazilian landline' do
        expect(PhoneFormatter.format('1133334444')).to eq('+551133334444')
      end

      it 'adds country code and formats correctly' do
        expect(PhoneFormatter.format('21991234567')).to eq('+5521991234567')
      end
    end

    context 'With country code - Story 2.2 Scenario 2' do
      it 'returns phone as-is when already in E.164' do
        expect(PhoneFormatter.format('+5511987654321')).to eq('+5511987654321')
      end

      it 'validates international format with country code' do
        expect(PhoneFormatter.format('+551133334444')).to eq('+551133334444')
      end

      it 'handles US numbers' do
        expect(PhoneFormatter.format('+12125550123')).to eq('+12125550123')
      end

      it 'handles UK numbers' do
        expect(PhoneFormatter.format('+442071838750')).to eq('+442071838750')
      end

      it 'handles Japan numbers' do
        expect(PhoneFormatter.format('+81312341234')).to eq('+81312341234')
      end
    end

    context 'Formatted input - Story 2.2 Scenario 3' do
      it 'removes parentheses and converts to E.164' do
        expect(PhoneFormatter.format('(11) 9 8765-4321')).to eq('+5511987654321')
      end
    ...
    context 'With spaces - Story 2.2 Scenario 4' do
      it 'handles country code with spaces' do
        expect(PhoneFormatter.format('+55 11 9 8765-4321')).to eq('+5511987654321')
      end

      it 'handles multiple space types' do
        expect(PhoneFormatter.format('+55 11 98765 4321')).to eq('+5511987654321')
      end
    ...
    context 'Old format - Story 2.2 Scenario 5' do
      it 'converts old Brazilian format with leading zero' do
        expect(PhoneFormatter.format('011987654321')).to eq('+5511987654321')
      end

      it 'handles landline with leading zero' do
        expect(PhoneFormatter.format('01133334444')).to eq('+551133334444')
      end

      it 'removes only leading zero, keeps digits' do
        expect(PhoneFormatter.format('0219912345')).to eq('+5521991234')
      end
    end

    context 'Invalid - too short - Story 2.2 Scenario 6' do
      it 'raises error for very short number' do
        expect { PhoneFormatter.format('999') }
          .to raise_error(PhoneFormatter::PhoneFormatterError, /Invalid phone.*too short/)
      end

      it 'raises error for single digit' do
        expect { PhoneFormatter.format('5') }
          .to raise_error(PhoneFormatter::PhoneFormatterError, /Invalid phone.*too short/)
      end
      ...
      it 'includes original phone in error message' do
        expect { PhoneFormatter.format('123') }
          .to raise_error(PhoneFormatter::PhoneFormatterError, /Invalid phone: 123/)
      end
    end

    context 'Invalid - too long - Story 2.2 Scenario 7' do
      it 'raises error for excessively long number' do
        expect { PhoneFormatter.format('111111111111111111') }
      ...
      it 'raises error for number > 15 digits after country code' do
        expect { PhoneFormatter.format('+5511999999999999999') }
          .to raise_error(PhoneFormatter::PhoneFormatterError, /Invalid phone.*too long/)
      end

      it 'includes original phone in error message' do
        expect { PhoneFormatter.format('111111111111111111') }
          .to raise_error(PhoneFormatter::PhoneFormatterError, /Invalid phone: 111111111111111111/)
      end
    end

    context 'Invalid - letters - Story 2.2 Scenario 8' do
      it 'raises error for letters in phone' do
        expect { PhoneFormatter.format('abc123') }
          .to raise_error(PhoneFormatter::PhoneFormatterError, /Invalid phone.*invalid format/)
      end

      it 'raises error for mixed alphanumeric' do
        expect { PhoneFormatter.format('+55abc123') }
          .to raise_error(PhoneFormatter::PhoneFormatterError, /Invalid phone.*invalid format/)
      end
    ...
    context 'WhatsApp normalization - Brazil' do
      it 'adds 9 digit for old Brazilian format (55 + DDD + 8 digits)' do
        # Input: 551187654321 (11 digits) = 55 + 11 + 87654321
        # Output: 5511987654321 (13 digits) = 55 + 11 + 9 + 87654321
        expect(PhoneFormatter.format('551187654321')).to eq('+5511987654321')
      end

      it 'does not modify when already has 9 digit' do
        # Input: 5511987654321 (13 digits) = 55 + 11 + 9 + 87654321
        # Output: same
        expect(PhoneFormatter.format('5511987654321')).to eq('+5511987654321')
      end
      ...
      it 'handles formatted old format' do
        ...
        # Output: +5511987654321 with 9 added
        expect(PhoneFormatter.format('(11) 8765-4321')).to eq('+5511987654321')
      end
    end

    context 'Edge cases' do
      it 'returns nil for empty string' do
        expect(PhoneFormatter.format('')).to be_nil
      end

      it 'returns nil for nil' do
        expect(PhoneFormatter.format(nil)).to be_nil
      end
    ...
    context 'International numbers' do
      it 'handles France numbers' do
        expect(PhoneFormatter.format('+33123456789')).to eq('+33123456789')
      end

      it 'handles Germany numbers' do
        expect(PhoneFormatter.format('+491234567890')).to eq('+491234567890')
      end

      it 'handles Canada numbers' do
        expect(PhoneFormatter.format('+14165551234')).to eq('+14165551234')
      end

      it 'handles Australia numbers' do
        expect(PhoneFormatter.format('+61212345678')).to eq('+61212345678')
      end

      it 'handles India numbers' do
        expect(PhoneFormatter.format('+919876543210')).to eq('+919876543210')
      end
    end

    context 'Different regional formats' do
      it 'handles São Paulo area code' do
        expect(PhoneFormatter.format('1191234567')).to eq('+551191234567')
      end

      it 'handles Rio de Janeiro area code' do
        expect(PhoneFormatter.format('2191234567')).to eq('+552191234567')
      end

      it 'handles Bahia area code' do
        expect(PhoneFormatter.format('7191234567')).to eq('+557191234567')
      end

      it 'handles different area codes' do
        expect(PhoneFormatter.format('4591234567')).to eq('+554591234567')
      end
    end
    ...
    context 'Error message quality' do
      it 'includes original phone value in error for invalid phone' do
        original = '(11) 1234'
        expect { PhoneFormatter.format(original) }
  ...
  describe 'integration with existing test expectations' do
    ...
    it 'handles all 8 story scenarios correctly' do
      scenarios = [
        ['11987654321', '+5511987654321'],               # Scenario 1
        ['+5511987654321', '+5511987654321'],           # Scenario 2
        ['(11) 9 8765-4321', '+5511987654321'],         # Scenario 3
        ['+55 11 9 8765-4321', '+5511987654321'],       # Scenario 4
        ['11 98765-4321', '+5511987654321'],            # Scenario 5
      ]

      scenarios.each do |input, expected|
        expect(PhoneFormatter.format(input)).to eq(expected),
